...
# Split up the df
manxie_int <-
  split(manxie_df, manxie_df$trip_id) %>%
  
  # Cut trip sections after more than 300 minute gaps
...
# Distance to colony
manxie_int$PointDist <-
  PointDist(manxie_int,
            x = col_lon,
            y = col_lat)

# Write out .csv of manxie data to get envirmental variables
write.csv(x = manxie_int, file = "data/cleaned/manxie_ls_int_df.csv")

manx_env <- read.csv("data/env/ls_int_env.csv") %>%
  mutate(date_time = dmy_hms(date_time)) %>%
  with(., .[order(ID, date_time), ]) %>%
  select(-ID, -Latitude, -Longitude, -date_time)

# Order this dataframe
manxie_int <- with(manxie_int, manxie_int[order(ID, date_time), ])

manxie_df <- cbind(manxie_int, manx_env)
...
length(unique(manxie_ls$section))
length(unique(manxie_ls$ID))

# Save it for later
save(manxie_ls, file = "data/cleaned/manxie_ls_int.RData")
