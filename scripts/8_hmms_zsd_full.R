...
HMM_data <- manxie_ls_restricted[c("x", "y", "section",
  ...
  mutate(step = ifelse(step == 0, 1, step),
         ...
         day = as.numeric(format(date_time, format = "%j")))
...
manxie_ls_restricted2 <- split(manxie_ls_restricted, manxie_ls_restricted$trip_id) %>%
  lapply(., function(x){
...
manxie_ls_restricted2 %>%
  mutate(dive_bool = ifelse(dives > 0, 1, 0)) %>%
  filter(dive_bool > 0) %>%
ggplot() +
  geom_violin(aes(y = max_depth, x = as.factor(hmm_state), fill = as.factor(hmm_state)))

manxie_ls_restricted2 %>% filter(dives > 0) %>% select(hmm_state) %>% table()
manxie_ls_restricted2 %>% select(hmm_state) %>% table()

manxie_ls_restricted2 %>% filter(dives > 0) %>% select(hmm_state_zsd) %>% table()
manxie_ls_restricted2 %>% select(hmm_state_zsd) %>% table()

manxie_ls_restricted2 %>% filter(hmm_state != 2) %>%
  mutate(dive_bool = ifelse(dives > 0, 1, 0)) %>% select(dive_bool) %>% table()
manxie_ls_restricted2 %>% filter(hmm_state == 2) %>%
  mutate(dive_bool = ifelse(dives > 0, 1, 0)) %>% select(dive_bool) %>% table()
manxie_ls_restricted2 %>% filter(hmm_state_zsd != 2) %>%
  mutate(dive_bool = ifelse(dives > 0, 1, 0)) %>% select(dive_bool) %>% table()
manxie_ls_restricted2 %>% filter(hmm_state_zsd == 2) %>%
  mutate(dive_bool = ifelse(dives > 0, 1, 0)) %>% select(dive_bool) %>% table()

sum(manxie_ls_restricted2$hmm_state == manxie_ls_restricted2$hmm_state_zsd) /
  nrow(manxie_ls_restricted2)

manxie_ls_restricted2 %>% filter(hmm_state_zsd == 1) %>% select(dives) %>% sum()
manxie_ls_restricted2 %>% filter(hmm_state_zsd == 2) %>% select(dives) %>% sum()
manxie_ls_restricted2 %>% filter(hmm_state_zsd == 3) %>% select(dives) %>% sum()

sum(manxie_ls_restricted2$dives)

ggplot(manxie_ls_restricted2) +
