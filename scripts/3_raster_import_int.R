...
# Load in the interpolated dataset
load(file = "data/cleaned/manxie_ls_int.RData")
...
for(i in 1:length(date_seq)){
  ...
  if(length(index) > 0){
    ...
    sp <- SpatialPoints(manxie_ls[index, c("Longitude", "Latitude")],
                        proj4string = CRS("+proj=longlat +datum=WGS84"))
    # Append each raster layer to manxie_ls using spatial points
    manxie_ls$zsd[index] <- raster::extract(zsd_old[[i]], sp)}
}

manxie_ls$HOD <- as.numeric(format(manxie_ls$date_time, format = "%H"))
manxie_ls$MOD <-
  (as.numeric(format(manxie_ls$date_time, format = "%M")) / 60) + 
  manxie_ls$HOD
